...
bool Odometry::update(double left_front_vel, double left_rear_vel, double right_front_vel, double right_rear_vel, const rclcpp::Time & time)
{
  ...
  // if (std::fabs(linear_x) > 0)
  //   RCLCPP_INFO(rclcpp::get_logger("Odometry"), "linear_x : %lf", linear_x);
  // if (std::fabs(angular) > 0)
  //   RCLCPP_INFO(rclcpp::get_logger("Odemetry"), "angular : %lf", angular);
  ...
  // integrateExact(linear_x * dt, 0 * dt);
  ...
  linear_ = linear_accumulator_.getRollingMean() / 100;
  angular_ = angular_accumulator_.getRollingMean() / 100;

  if (std::fabs(linear_) > 0.000001)
    RCLCPP_INFO(rclcpp::get_logger("Odometry"), "linear_ : %lf", linear_);
  if (std::fabs(angular_) > 0.0000001)
    RCLCPP_INFO(rclcpp::get_logger("Odemetry"), "angular_ : %lf", angular_);
