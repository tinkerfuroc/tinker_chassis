...
{      
    ...
    // RCLCPP_INFO(logger, "wheel_radius_: %lf",wheel_radius_);
    // RCLCPP_INFO(logger, "wheel_separation_width: %lf",wheel_separation_width_);
    // RCLCPP_INFO(logger, "wheel_separation_length_: %lf",wheel_separation_length_);
    // RCLCPP_INFO(logger, "twist_angular_z: %lf", twist.angular.z);
    if (abs(fl_wheel_velocity) > 0)
      RCLCPP_INFO(logger, "fl_wheel: %lf",fl_wheel_velocity);
    if (abs(fr_wheel_velocity) > 0)
      RCLCPP_INFO(logger, "fr_wheel: %lf",fr_wheel_velocity);
    if (abs(rl_wheel_velocity) > 0)
      RCLCPP_INFO(logger, "rl_wheel: %lf",rl_wheel_velocity);
    if (abs(rr_wheel_velocity) > 0)
      RCLCPP_INFO(logger, "rr_wheel: %lf",rr_wheel_velocity);
